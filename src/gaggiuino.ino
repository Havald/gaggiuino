...
static void sensorsReadPressure(void) {
  if (millis() > pressureTimer) {
    ...
    currentState.pressureDelta = refreshPressureDelta();
...
static void refreshFlowState(unsigned long elapsedTime) {
  long pumpClicks = getAndResetClickCounter();
  float cps = 1000.f * pumpClicks / elapsedTime;
  float lastFlow = currentState.pumpFlow;
  currentState.pumpFlow = getPumpFlow(cps, currentState.pressure);
  currentState.flowDelta = currentState.pumpFlow - lastFlow;
  currentState.liquidPumped += currentState.pumpFlow * elapsedTime / 1000;
  currentState.isOutputFlow = checkForFlowOutput();
}

bool checkForFlowOutput(void) {
  unsigned long timeInPP = millis() - brewingTimer;
  CurrentPhase currentPhase = phases.getCurrentPhase(timeInPP);
  Phase current = phases.phases[currentPhase.phaseIndex];

  if (!preinfusionFinished) {
    if (currentState.liquidPumped > 40.f && currentState.pressure < 1.f)
      return true;
  } else if (phases.phases[currentPhase.phaseIndex].type == PHASE_TYPE_PRESSURE) {
    if (currentState.pressureDelta > 0.03f && currentState.flowDelta >= 0 || currentState.flowDelta < -0.05) {
      return false;
    } else
      return true;
  } else {
    if (currentState.pressureDelta > 0.03f && currentState.flowDelta <= 0.01)
      return false;
    else if (currentState.flowDelta < 0.1 && currentState.flowDelta > -0.1)
      return true;
  }
  return false;
}
...
bool stopOnWeight() {
  shotTarget = runningCfg.shotStopOnCustomWeight ? runningCfg.shotStopOnCustomWeight : (runningCfg.shotDose * runningCfg.shotPreset);
